...
<html>
    ...
    <%
        try {
            ...
            // NOUVELLE ACTION : Update uniquement les filles sans toucher à la mère
            if (acte != null && acte.compareToIgnoreCase("updateFille") == 0) {
                mere = (ClassMAPTable) (Class.forName(classe).newInstance());
                fille = (ClassMAPTable) (Class.forName(classefille).newInstance());
                PageUpdateMultiple p = new PageUpdateMultiple(mere, fille, request, nbLine, tId);
                
                // Récupérer seulement :l'ID de la mère
                idmere = request.getParameter("id");
                
                // Calculer le montant total AVANT modification
                ...
                // Calculer le montant total APRES modification
        ...
        <%
                ...
                java.util.ArrayList<ClassMAPTable> nouvelles = new java.util.ArrayList<ClassMAPTable>();
                java.util.ArrayList<ClassMAPTable> existantes = new java.util.ArrayList<ClassMAPTable>();
                
                for (int i = 0; i < cfille.length; i++) {
                    cfille[i].setNomTable("VENTE_DETAILS");
                    cfille[i].setValChamp(colonneMere, idmere);
                    
                    // Définir des valeurs par défaut pour les nouvelles lignes (toujours)
